...
namespace EventHUD.Ai
{
    public static class AiContextBuilder
    {
        ...
        private static void AppendPlayerInfo(StringBuilder sb, Player player)
        {
            ...
            sb.AppendLine($"Игрок, который с тобой говорит: ник {player.Nickname}, его ID: {player.Id}, ранг {badge}, класс {player.Role.Type}, здоровье {(int)player.Health} HP, инвентарь: {items}.");
        }
        ...
        private static void AppendPermissionInfo(StringBuilder sb, AiPermissionLevel level, Config cfg)
        {
            ...
            sb.AppendLine("Цели команд: пиши {id} чтобы указать игрока, который с тобой говорит (это заменится на его ID автоматически), либо конкретный ID. Можно несколько целей через запятую (2,5,7), ник игрока, me или all (все на сервере).");

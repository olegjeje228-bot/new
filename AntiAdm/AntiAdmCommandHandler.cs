...
namespace EventHUD.AntiAdm
{
    ...
    public class AntiAdmCommandHandler
    {
        ...
        // Map editor (MER)
        private readonly Dictionary<string, List<DateTime>> _mpCrTimes = new();
        private readonly Dictionary<string, int> _mpLoadCount = new();
        ...
        private static readonly HashSet<string> HeavyCommands = new()
        {
            ...
            "changescale", "scale", "ccolor", "color", "server_event",
        ...
        public void OnSendingValidCommand(SendingValidCommandEventArgs ev)
        {
            ...
            // ── MER: отдельные правила, остальные проверки АА к mp не применяются ──
            if (cmd == "mp" || cmd == "mapeditor")
            {
                HandleMapEditor(ev, args, senderId);
                return;
            }
        ...
        private const int MpCrPerMinute = 10;
        private const int MpLoadMax = 5;
        private const int MerFreeKickPower = 40;
        private const int BuilderKickPower = 11;

        private int GetKickPower(string senderId)
        {
            if (string.IsNullOrEmpty(senderId) || senderId == "console")
                return int.MaxValue;
            ...
            try
            {
                var sr = player.ReferenceHub.serverRoles;
                var prop = sr.GetType().GetProperty("KickPower",
                    System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic |
                    System.Reflection.BindingFlags.Instance);
                if (prop != null)
                    return Convert.ToInt32(prop.GetValue(sr));

                var field = sr.GetType().GetField("_kickPower",
                    System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                if (field != null)
                    return Convert.ToInt32(field.GetValue(sr));
            }
            catch { }
        ...
        private void HandleMapEditor(SendingValidCommandEventArgs ev, string[] args, string senderId)
        {
            int kickPower = GetKickPower(senderId);

            if (kickPower == BuilderKickPower || kickPower > MerFreeKickPower)
                return;

            string sub = args.Length > 0 ? args[0].ToLowerInvariant() : string.Empty;

            if (sub == "unl" || sub == "unload")
            {
                _mpLoadCount[senderId] = 0;
                return;
            }

            if (sub == "cr" || sub == "create")
            {
                if (!_mpCrTimes.TryGetValue(senderId, out var times))
                {
                    times = new List<DateTime>();
                    _mpCrTimes[senderId] = times;
                }
                var cutoff = DateTime.UtcNow.AddSeconds(-60);
                times.RemoveAll(t => t < cutoff);
                if (times.Count >= MpCrPerMinute)
                {
                    Deny(ev, "AA-12");
                    return;
                }
                ...
                Plugin.Instance?.AntiLag?.TemporarilyDisable(60f);
                return;
            }

            if (sub == "load" || sub == "l")
            {
                _mpLoadCount.TryGetValue(senderId, out int used);
                if (used >= MpLoadMax)
                {
                    Deny(ev, "AA-12");
                    return;
                }
                _mpLoadCount[senderId] = used + 1;
